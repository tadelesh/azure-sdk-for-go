...
// More returns true if there are more pages to retrieve.
func (p *RequestStatusClientListPager) More() bool {
	if !reflect.ValueOf(p.current).IsZero() {
...
// NextPage advances the pager to the next page.
func (p *RequestStatusClientListPager) NextPage(ctx context.Context) (RequestStatusClientListResponse, error) {
	var req *policy.Request
	var err error
	if !reflect.ValueOf(p.current).IsZero() {
		if !p.More() {
			return RequestStatusClientListResponse{}, errors.New("no more pages")
		}
		req, err = p.advancer(ctx, p.current)
	} else {
		req, err = p.requester(ctx)
