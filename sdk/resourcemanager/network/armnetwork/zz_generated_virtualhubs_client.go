...
// method.
func (client *VirtualHubsClient) BeginGetEffectiveVirtualHubRoutes(ctx context.Context, resourceGroupName string, virtualHubName string, options *VirtualHubsClientBeginGetEffectiveVirtualHubRoutesOptions) (*armruntime.Poller[VirtualHubsClientGetEffectiveVirtualHubRoutesResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.getEffectiveVirtualHubRoutes(ctx, resourceGroupName, virtualHubName, options)
		if err != nil {
			return nil, err
		}
		return armruntime.NewPoller[VirtualHubsClientGetEffectiveVirtualHubRoutesResponse]("VirtualHubsClient.GetEffectiveVirtualHubRoutes", "location", resp, client.pl, nil)
	} else {
		return armruntime.NewPollerFromResumeToken[VirtualHubsClientGetEffectiveVirtualHubRoutesResponse]("VirtualHubsClient.GetEffectiveVirtualHubRoutes", options.ResumeToken, client.pl, nil)
	}
}
