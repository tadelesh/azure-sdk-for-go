...
func ExampleCapacityReservationGroupsClient_ListBySubscription() {
	...
	ctx := context.Background()
	client := armcompute.NewCapacityReservationGroupsClient("<subscription-id>", cred, nil)
	pager := client.ListBySubscription(&armcompute.CapacityReservationGroupsClientListBySubscriptionOptions{Expand: to.Ptr(armcompute.ExpandTypesForGetCapacityReservationGroupsVirtualMachinesRef)})
	for pager.More() {
		nextResult, err := pager.NextPage(ctx)
		if err != nil {
			log.Fatalf("failed to advance page: %v", err)
